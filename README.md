...
SysRec はコード署名を行っていないため、インストール時に警告が表示されることがあります。

### Windows の警告（SmartScreen）

インストーラーを実行すると、下記のような画面が表示される場合があります。

> 「Windows によって PC が保護されました」

この場合は **「詳細情報」** をクリックし、**「実行」** を押してください。

### セキュリティソフトの警告

ウイルス対策ソフトが「脅威を検出した」と警告を表示することがあります。  
これは署名のない実行ファイルに対して多くのセキュリティソフトが出す誤検知です。  
警告が出た場合は **「許可」または「除外に追加」** を選択してください。

> **ご不安な方へ：** ソースコードは [GitHub](https://github.com/workntones-cyber/SysRec) で公開しており、内容をご確認いただけます。
